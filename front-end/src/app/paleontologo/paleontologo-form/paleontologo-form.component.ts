...
@Component({
  selector: 'app-paleontologo-form',
  templateUrl: './paleontologo-form.component.html',
  styleUrls: ['./paleontologo-form.component.scss']
})
export class PaleontologoFormComponent implements OnInit {

  title : string = 'Novo paleontólogo'

  paleontologo : any = {} // Objeto vazio, nome da entidade no SINGULAR
  ...
  async ngOnInit() {
    ...
    if(this.actRoute.snapshot.params['id']) {
      try {
        ...
        this.paleontologo = await this.paleontologoSrv.obterUm(this.actRoute.snapshot.params['id'])
        // 2) Mudar o título da página
        this.title = 'Editando paleontólogo'
      }
      catch(erro) {
        console.log(erro)
  ...
  async salvar(form : NgForm) {
    try {
      if(form.valid) {
        ...
        if(this.paleontologo._id) {
          // _id existe, esse registro já foi salvo anteriormente
          // no BD e é caso de atualização
          await this.paleontologoSrv.atualizar(this.paleontologo)
        }
        else {
          await this.paleontologoSrv.novo(this.paleontologo)
        }
